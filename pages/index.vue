<template>
  <div class="min-h-screen dark:bg-gray-900 bg-gray-50">
    <div v-if="isLoadingStats || isLoadingServers || isLoadingSoftware"
      class="flex items-center justify-center min-h-screen">
      <Spinner />
    </div>

    <ErrorNetworkFailure v-else-if="isErrorStats || isErrorServers || isErrorSoftware"
      :show="isErrorStats || isErrorServers || isErrorSoftware"
      :error="statsError?.message || serversError?.message || softwareErrors?.message || 'Something went wrong.'"
      @retry="retryAll" :show-go-back="false" />
    ...
    <main v-else class="container max-w-6xl mx-auto px-4 py-8 dark:text-white">
      <section class="mb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          <StatCard title="Active Accounts" :value="stats.monthly_active_users.toLocaleString()" icon="active" />
          <StatCard title="Active Servers" :value="stats.total_instances.toLocaleString()" icon="server" />
          <StatCard title="Total Accounts" :value="formatNumber(stats.total_users)" icon="users" />
          <StatCard title="Total Statuses" :value="formatNumber(stats.total_statuses)" icon="posts" />
        </div>
      ...
      <section class="mb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <SoftwareGridCard v-for="software in softwarePlatforms.slice(0, 6)" :key="software.id" :software="software"
      ...
      <section class="mb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <ServerCard v-for="server in topServers.slice(0, 6)" :key="server.id" :name="server.description"
            :domain="server.domain" :users="server.stats.user_count.toLocaleString()"
            :software="server.software?.name || 'Unknown software'" :uptime="server.uptime"
            :thumbnail="server.thumbnail" :description="server.description || 'No server description available'" />
        </div>
      </section>
    </main>
  </div>
