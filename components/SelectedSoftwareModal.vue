<template>
    <div v-if="selectedSoftware" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-900/80 backdrop-blur-sm transition-opacity -z-10" aria-hidden="true"
            ...
            <div
                ...
                <div class="p-6">
                    <div class="flex flex-wrap md:flex-nowrap">
                        <div class="w-full mb-6 md:mb-0" :class="[hasRichGraphs ? 'md:w-1/3 md:pr-6' : 'md:px-5']">
                            <div class="flex items-center mb-4">
                                <div class="h-12 w-12 rounded-lg mr-3"
                                    :class="selectedSoftware.logo_url ? '' : getSoftwareBgColor(selectedSoftware.name)">
                                    <div class="h-full w-full flex items-center justify-center">
                                        <img v-if="selectedSoftware.logo_url" :src="selectedSoftware.logo_url" alt=""
                                            class="h-10 w-10 overflow-hidden  object-scale-down">
                                        <span v-else class="text-xl font-bold text-white">{{
                                            selectedSoftware.name.charAt(0) }}</span>
                                    </div>
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-6">
                                <div v-if="selectedSoftware?.latest_version?.version">
                                    ...
                                    <p class="dark:text-gray-200 font-medium">{{
                                        selectedSoftware?.latest_version?.version || 'Unknown' }}</p>
                                </div>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Monthly Active
                                        Users</h4>
                                    <p class="dark:text-gray-200 font-medium">{{
                                        formatNumber(selectedSoftware?.monthly_actives || 0) }}</p>
                                </div>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Servers</h4>
                                    <p class="dark:text-gray-200 font-medium">{{
                                        selectedSoftware?.instance_count?.toLocaleString() || 0 }}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p class="dark:text-gray-200 font-medium">{{ selectedSoftware?.license || 'Unknown'
                                        }}</p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="space-y-4">
                                    <div v-if="selectedSoftware.website" class="flex items-center gap-4">
                                        ...
                                        <a :href="selectedSoftware.website"
                                    ...
                                    <div v-if="selectedSoftware.apps_url" class="flex items-center gap-4">
                                        <Icon name="feather:tablet" size="25"
                                            class="text-gray-400 dark:text-gray-500" />
                                        <a :href="selectedSoftware.apps_url"
                                            class="text-blue-600 dark:text-blue-400 hover:underline" target="_blank"
                                            rel="noopener noreferrer">Apps and Clients</a>
                                    </div>

                                    <div v-if="selectedSoftware.forum_url" class="flex items-center gap-4">
                                        <Icon name="feather:message-square" size="25"
                                            class="text-gray-400 dark:text-gray-500" />
                                        <a :href="selectedSoftware.forum_url"
                                            class="text-blue-600 dark:text-blue-400 hover:underline" target="_blank"
                                            rel="noopener noreferrer">Discussions</a>
                                    </div>

                                    <div v-if="selectedSoftware.source_repo" class="flex items-center gap-4">
                                        <Icon name="feather:git-pull-request" size="25"
                                            class="text-gray-400 dark:text-gray-500" />
                                        <a :href="selectedSoftware.source_repo"
                                            class="text-blue-600 dark:text-blue-400 hover:underline" target="_blank"
                                            rel="noopener noreferrer">{{ sourceRepoDomain }} Source</a>
                                    </div>
                                    ...
                                    <div v-if="selectedSoftware.api_docs_url" class="flex items-center gap-4">
                                        ...
                                        <a :href="selectedSoftware.api_docs_url"
                                            class="text-blue-600 dark:text-blue-400 hover:underline" target="_blank"
                        ...
                        <div v-if="hasRichGraphs" class="w-full md:w-2/3">
                            <div v-if="selectedSoftware.historical_stats?.length"
                                class="border border-gray-100 mb-6 dark:border-gray-700 rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-900">
                                <ReusableChart title="Activity" :chart-data="selectedSoftware.historical_stats"
                                    x-axis-field="period" :x-axis-formatter="formatGraphDate" :series="seriesConfig" />
                            </div>
                            <div class="flex grow gap-2">
                                <div v-if="selectedSoftware.versions?.length"
                                    class="h-auto grow border border-gray-100 dark:border-gray-700 rounded-lg overflow-hidden bg-gray-50 dark:bg-gray-900 p-4">
                                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Version
                                        History</h4>
                                    <div class="space-y-3">
                                        <div v-for="(version, index) in selectedSoftware.versions.slice(0, 5)"
                                            :key="index"
                                            class="pt-3 border-t border-gray-200 dark:border-gray-700 first:border-0">
                                            <div class="flex justify-between items-center gap-3">
                                                <div class="text-xs md:text-sm font-medium dark:text-white">v{{
                                                    truncateText(version.version, 12) }}</div>
                                                <div class="text-xs text-gray-500 dark:text-gray-400">{{
                                                    version.count?.toLocaleString() }} servers ({{
                                                        Math.round(version.count / selectedSoftware.instance_count * 100)
                                                    }}%)</div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div v-if="selectedSoftware?.popular_asns?.length"
                                    class="h-auto grow border border-gray-100 dark:border-gray-700 rounded-lg overflow-hidden bg-gray-50 dark:bg-gray-900 p-4">
                                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Popular ASNs
                                    </h4>
                                    <div class="space-y-3">
                                        <div v-for="(asn, index) in selectedSoftware.popular_asns" :key="index"
                                            class="pt-3 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 first:border-0">
                                            <div class="flex">

                                                <div
                                                    class="h-6 w-6 rounded bg-gray-200 dark:bg-gray-700 mr-2 flex items-center justify-center text-xs">
                                                    {{ asn.asn_org.charAt(0).toUpperCase() }}
                                                </div>
                                                <span class="text-sm truncate dark:text-gray-200">{{ asn.asn_org
                                                    }}</span>
                                            </div>
                                            <span class="text-xs text-gray-500 dark:text-gray-400">{{
                                                asn.count.toLocaleString() }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <PrimaryBtn v-if="selectedSoftware.join_url" :name="`Join ${selectedSoftware.name}`"
                                :path="selectedSoftware.join_url" :fullWidth="true" class="mt-6" />
                        </div>
                    </div>
                </div>

                <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
                    <NuxtLink :href="`/software/${selectedSoftware.slug}`"
                        class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-medium">
                        View All Servers Using {{ selectedSoftware.name }} →
                    </NuxtLink>
                    <button @click="handleClose"
                        class="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-lg transition-colors">
                        Close
                    </button>
                </div>
            </div>
